...
cord = '''turn on 887,9 through 959,629
...
'''fteste = turn off 499,499 through 500,500
turn on 0,0 through 999,999
toggle 0,0 through 999,0'''
#teste = fteste.replace(" ","").split()

lista = cord.replace(" ","").split()
#print(len(lista))
lampada = {}
status = 'off'
cont = 0
...
# Criar o dicionario com todas as lampadas 
...
for anda in lista:
    print(cont)
    on = re.findall(r'turnon',lista[cont])
    off = re.findall(f'turnoff', lista[cont])
    troca = re.findall(f'toggle', lista[cont])
    match = re.search(r'(\d+),(\d+).*?(\d+),(\d+)', lista[cont])
    cont += 1
    ix=int(match.group(1))
    iy=int(match.group(2))
    fx=int(match.group(3))
    fy=int(match.group(4))
    print(f'{ix},{iy} through {fx},{fy}',end=" ")
    

    if on:
        print('ligar')
        for c in range(ix,fx+1):
            for i in range(iy,fy+1):
                y = i
                #print(f'Esse ligou{c},{y}') 
                lampada[c,y] = 'on'
        ...
        for c in range(ix,fx+1):
            for i in range(iy,fy+1):
                y = i
            if lampada[c,y] == 'on':
                #print(f'esse trocou para desligado {c},{y}')
                lampada[c,y] = 'off'
            
            else:                 
                #print(f'Esse trocou para ligado{c},{y}')   
                lampada[c,y] = 'on'

# contar lampadas acesas
lampOn = 0
for c in lampada.values():
    if c == 'on':
        lampOn += 1
