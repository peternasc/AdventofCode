...
# Criar o dicionário com todas as lâmpadas
lampada = {}
for x in range(1000):
    for y in range(1000):
        lampada[(x, y)] = 0
...
lista = cord.splitlines()

for c in lista:
    #print(c)
    match = re.match(r'(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)', c)
    
    if match:
        acao, x1, y1, x2, y2 = match.groups()
        x1, y1, x2, y2 = map(int, (x1, y1, x2, y2))
        
        # Atualizar o status das lâmpadas conforme o comando
    ...
    #brilho = sum(lampada.values())
    #print(f'valor total do brilho: {brilho}')
# Contar quantas lâmpadas estão ligadas
brilho = sum(lampada.values())
print(f'valor total do brilho: {brilho}')
